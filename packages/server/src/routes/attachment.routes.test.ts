...
describe('Attachment Routes', () => {
  ...
  describe('DELETE /api/attachments/:id', () => {
    it('성공: 삭제된 attachment id 반환', async () => {
      mockAttachmentService.delete.mockResolvedValue({ id: 'attachment-1' });
      ...
      expect(res.body).toEqual({ id: 'attachment-1' });
      expect(mockAttachmentService.delete).toHaveBeenCalledWith(
        'user-1',
        'attachment-1',
      );
    });
    ...
    it('첨부파일이 없으면 404', async () => {
      const { NotFoundError } = await import('../utils/errors.js');
      mockAttachmentService.delete.mockRejectedValue(
        new NotFoundError('Attachment'),
      );

      const res = await request(app)
        .delete('/api/attachments/no-attachment')
        .set('Authorization', `Bearer ${token}`);
    ...
    it('소유자가 아니면 403', async () => {
      ...
      mockAttachmentService.delete.mockRejectedValue(new ForbiddenError());
